...
export default function ClientCenterViewFaq({ route, navigation }) {
    const [faqList, setFaqList] = useState()


    // 자주 묻는 질문 및 답변 목록 가져오기
    const getFaqList = () => {
        showPageLoader(true)
        RestAPI.getFaq().then(res => {
            setFaqList(res)
        }).catch(err => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <Header
                ...
                centerComponent={
                    <Text style={{ fontSize: 16, fontWeight: 'bold', }}>자주 묻는 질문</Text>
                }
                backgroundColor="#FFF"
                containerStyle={{ 
